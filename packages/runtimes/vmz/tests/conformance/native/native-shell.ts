/**
 * native: Native WebView shell + local bundled Browser Direct entry +
...
console.log('native gate: native-host catalog includes shell documents…');
...
console.log('native gate: Browser Direct build + write native-shell.json…');
...
console.log('native gate: checkNativeShellContractJson…');
const report = JSON.parse(checkNativeShellContractJson(dir));
...
const remoteReport = JSON.parse(checkNativeShellContractJson(dir));
...
console.log('native gate: reject missing shell hook…');
...
const hookReport = JSON.parse(checkNativeShellContractJson(dir));
...
console.log('native gate: reject platform schema fork…');
...
const forkReport = JSON.parse(checkNativeShellContractJson(dir));
...
console.log('native gate: reject missing entry artifact…');
fs.writeFileSync(path.join(dir, 'native-shell.json'), JSON.stringify(shell, null, 2));
fs.unlinkSync(clientJs);
const missing = JSON.parse(checkNativeShellContractJson(dir));
...
console.log('native GATE OK: shell + local bundle + hooks + ios/android shared schema + Browser Direct entry + foul rejects');
