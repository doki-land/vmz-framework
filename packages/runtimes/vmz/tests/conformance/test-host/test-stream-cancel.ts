/**
 * stream cancel / backpressure gate:
...
function fail(msg) {
    console.error(`-STREAM-CANCEL GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('-stream-cancel: vmz test --mode ssr --filter counter.ssr.stream.cancel…');
const run = spawnSync(
    process.execPath,
...
console.log('-STREAM-CANCEL GATE PASS');
