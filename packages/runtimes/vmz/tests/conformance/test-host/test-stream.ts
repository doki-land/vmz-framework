/**
 * stream SSR gate: renderToStream join === renderToString (Direct schedule).
 */
...
function fail(msg) {
    console.error(`-STREAM GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('-stream gate: vmz test --mode ssr --filter counter.ssr.stream…');
const run = spawnSync(
    process.execPath,
