/**
 * symbols: cross-SFC Symbol/Reference + method/component rename + source map + safe_fix.
 *
 *
 */
...
console.log('symbols: DX catalog includes source_map / symbol_index / cross_sfc_check…');
...
for (const [kind, schema] of [
    ...
    ['cross_sfc_check', DX_CROSS_SFC_CHECK_SCHEMA],
...
console.log('symbols: checkCrossSfc / querySymbols / source map…');
if (typeof ws.checkCrossSfc !== 'function') fail('checkCrossSfc missing');
const report = JSON.parse(ws.checkCrossSfc());
if (report.schema !== DX_CROSS_SFC_CHECK_SCHEMA) fail(`report schema ${report.schema}`);
...
console.log('symbols: method + component rename apply…');
...
console.log('symbols: capability rename…');
...
console.log('symbols: queryReferences + safe_fix CodeAction…');
...
console.log(' GATE OK: cross-SFC symbols + rename + source map + safe_fix');
