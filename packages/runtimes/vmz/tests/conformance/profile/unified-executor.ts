/**
 * unified-executor: Unified Executor — same transaction across Surfaces,
 * generation discard, DisposeRegion / cancel (architecture notes / ).
 *
...
console.log('unified-executor: catalog includes executor documents…');
...
console.log('unified-executor: Browser Direct smoke…');
...
console.log('unified-executor: checkUnifiedExecutorJson (mixed camera → T42)…');
const report = JSON.parse(checkUnifiedExecutorJson(dir));
...
const wsReport = JSON.parse(ws.checkUnifiedExecutor());
...
console.log('unified-executor: reject missing envelope ids…');
...
const missFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject stale generation…');
...
const staleFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject surface owns state…');
...
const ownFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject private object crossing…');
...
const privFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject split transaction…');
...
const splitFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject dispose not authoritative…');
...
const dispFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log('unified-executor: reject cancel not propagated…');
...
const cancelFail = JSON.parse(checkUnifiedExecutorJson(dir));
...
console.log(' GATE OK: Unified Executor same-tx across Surfaces + generation/cancel/dispose');
