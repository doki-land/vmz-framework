...
addLimitation(proof, 'server-host: live Deno/Bun/Cloudflare Worker runtimes not covered (in-process Fetch parity only)');
addLimitation(proof, 'server-host: middleware contribution / CORS-CSRF-cache matrix not covered');
addLimitation(proof, 'server-host: live Rust host binary parity not covered (contract projection only)');
writeProof(proof, root);

console.log(
    `server-host PASS: ServerArtifact + public/internal isolation + Node/Fetch parity (digest=${String(artifact.artifactDigest).slice(0, 12)})`,
);
