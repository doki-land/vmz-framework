/**
 * direct-emit (thin): compile half + ssr/hydrate mode.
 */
...
function fail(msg) {
    console.error(`direct-emit FAIL: ${msg}`);
    process.exit(1);
}

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'vmz-direct-'));
const reportPath = path.join(tmp, 'report.json');

console.log('direct-emit: vmz test compile + ssr…');
const run = spawnSync(
    process.execPath,
...
console.log('direct-emit PASS: shared Plan + Direct mount/SSR/hydrate (no production render)');
