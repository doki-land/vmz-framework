...
import { assembleDelivery, emitBuildProof } from '../workspace/build-assemble.js';
import { resolveCliLocalesRoot } from './cli-localize.js';
import { normalizeDeliveryAuthoring, resolveProfileArtifactDir, selectBuildProfile } from '../delivery/delivery-profile.js';
import { createDevSession } from '../dev/dev-session.js';
import { registerDocumentCommands } from '../document/document-cmd.js';
import { buildIntegratedDocuments, projectHasDocuments } from '../document/document-integrate.js';
...
import {
    ...
    resolveNativePath,
} from '../workspace/public-api.js';
import { gateGlobalProjectCommand, getInvocationContext, isGlobalAllowedCommand } from '../workspace/invocation.js';
import { emitLocaleRuntimeModules, localeHasErrors } from '../locale/locale-check.js';
...
import { findAvailablePort } from '../workspace/port.js';
import { registerRefactorCommands } from './refactor-cmd.js';
import { registerArtifactCommands } from './release-cmd.js';
import { readPackageMeta, resolveWorkspaceDirs } from '../workspace/resolve.js';
import { emitRouteCatalog } from '../workspace/route-catalog-emit.js';
...
async function cmdCheck(args: ParsedOptions): Promise<number> {
    ...
    try {
        return runWithPlugins(ws, project, outDir, async () => {
            ...
            const { checkLocales, localeHasErrors } = await import('../locale/locale-check.js');
...
async function runWithPlugins(ws: Workspace, project: string, outDir: string, fn: () => Promise<number> | number): Promise<number> {
    const { loadVmzConfig, applyPlugins } = await import('../workspace/plugin-host.js');
