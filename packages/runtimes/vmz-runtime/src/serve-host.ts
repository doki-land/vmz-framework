...
function emitEntryClient(eager, lazy, token) {
    const q = `?t=${token}`;
    const native = tryLoadNativeGenerator();
    if (native?.generateServeEntryClient) {
...
function emitEntryEvent(token) {
    const q = `?t=${token}`;
    const native = tryLoadNativeGenerator();
    if (native?.generateServeEntryEvent) {
...
function tryLoadNativeGenerator() {
    if (_nativeGen !== undefined) return _nativeGen;
    const addon = process.env.VMZ_NATIVE_NODE;
    if (!addon) {
        _nativeGen = null;
        return null;
    }
    try {
        _nativeGen = require(addon);
        return _nativeGen;
    } catch {
        _nativeGen = null;
        return null;
