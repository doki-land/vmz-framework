...
fn reject_copies_semantic_ir() {
    ...
    assert_eq!(report.status, "failed");
    assert!(
        report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_PROOF_COPIES_SEMANTIC_IR))
    );
    let _ = fs::remove_dir_all(dir);
}
...
fn reject_update_without_reproof() {
    ...
    assert_eq!(report.status, "failed");
    assert!(
        report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_UPDATE_WITHOUT_REPROOF))
    );
    let _ = fs::remove_dir_all(dir);
}
...
fn reject_insecure_remote() {
    ...
    assert_eq!(report.status, "failed");
    assert!(
        report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_SECURITY_POLICY_INSECURE))
    );
    let _ = fs::remove_dir_all(dir);
}
