...
fn rejects_adapter_as_semantic_core() {
    ...
    assert_eq!(report.status, "failed");
    assert!(
        report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_ADAPTER_IS_SEMANTIC_CORE))
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn rejects_missing_android_adapter() {
    ...
    assert_eq!(report.status, "failed");
    assert!(
        report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_MISSING_PLATFORM_ADAPTER))
    );
    let _ = fs::remove_dir_all(&dir);
}
