//! Direct create/patch codegen from Native View (Program IR B / L2 / Gate 3).
//!
...
//! Production products do **not** emit blueprint `render()` (Gate 3 full close).
...
/// SSR reuses the same create function with a serialize host API (L3).
...
/// Emit `__vmzPlan` literal matching `units[].plan` in program.json (L3 shared identity).
...
mod tests {
    ...
    use crate::emit::emit_client_js;
    use crate::reactive_build::build_program_module;
...
"#;
        ...
        let tpl = parse_template(
            r#"<button type="button" onClick={() => count++}>count: {count}</button>"#,
        );
...
"#;
        let client = analyze_script(ScriptKind::Client, src);
        let tpl =
            parse_template(r#"<ul><li each={tags} as="tag" key={tag.id}>{tag.label}</li></ul>"#);
        let program = build_program_module("t.vmz", &client.decl, &tpl);
        assert!(is_direct_eligible(&program.units[0].view));
        let js = emit_client_js(src, &client, &tpl, None).unwrap();
