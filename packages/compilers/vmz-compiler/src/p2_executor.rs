//! P2: Unified Executor algebraic check (doc 13 §4.7 / §4.14).
//!
...
/// Validate an ExecutorScenario against P2 hard contracts.
...
/// P2 check for a workspace root (optional executor-scenario.json).
pub fn check_p2_unified_executor(root: &Path) -> ExecutorCheckReport {
...
mod tests {
    ...
    fn mixed_camera_t42_ready() {
        let dir = tmp("vmz-p2-");
        let report = check_p2_unified_executor(&dir);
    ...
    fn rejects_stale_generation_patches() {
        ...
        assert_eq!(report.status, "failed");
        assert!(
            report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_STALE_GENERATION))
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_surface_owns_state() {
        ...
        assert_eq!(report.status, "failed");
        assert!(
            report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_SURFACE_OWNS_STATE))
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_private_object_crossing() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_PRIVATE_OBJECT_CROSSING))
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_split_transaction() {
        ...
        assert_eq!(report.status, "failed");
        assert!(
            report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_SPLIT_TRANSACTION))
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_cancel_not_propagated() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_CANCEL_NOT_PROPAGATED))
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = (
