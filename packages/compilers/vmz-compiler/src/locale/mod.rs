    ...
    let text = match fs::read_to_string(&path) {
        ...
        Err(e) => {
            return LocalePlan {
                ...
                missing: "error".into(),
                diagnostics: vec![ReportedDiagnostic::coded_error(
                    rel,
                    format!("read locales manifest failed: {e}"),
                    DIAG_CATALOG_PARSE,
                )],
            };
        }
    ...
    let value: Value = match json5::from_str(source) {
        ...
        Err(e) => {
            return LocalePlan {
                ...
                diagnostics: vec![ReportedDiagnostic::coded_error(
                    source_path,
                    format!("locales.json5 parse failed: {e}"),
                    DIAG_CATALOG_PARSE,
                )],
            };
        }
    ...
    let file: LocaleManifestFile = match serde_json::from_value(value) {
        ...
        Err(e) => {
            return LocalePlan {
                ...
                diagnostics: vec![ReportedDiagnostic::coded_error(
                    source_path,
                    format!("locales.json5 shape invalid: {e}"),
                    DIAG_CATALOG_PARSE,
                )],
            };
        }
...
fn normalize_locale_manifest(file: LocaleManifestFile, source_path: String) -> LocalePlan {
    ...
    for entry in file.locales {
        ...
        match validate_locale_id(&id) {
            Ok(()) => {}
            Err(msg) => {
                diagnostics.push(ReportedDiagnostic::coded_error(
                    source_path.clone(),
                    msg,
                    DIAG_LOCALE_ID_INVALID,
                ));
                continue;
            }
        }
        if !seen.insert(id.clone()) {
            diagnostics.push(ReportedDiagnostic::coded_error(
                source_path.clone(),
                format!("duplicate LocaleId {id} in locales[]"),
                DIAG_LOCALE_ID_COLLISION,
            ));
            continue;
        }
    ...
    for (from, chain) in file.fallback {
        if !seen.contains(&from) {
            diagnostics.push(ReportedDiagnostic::coded_error(
                source_path.clone(),
                format!("fallback key {from} is not a declared LocaleId"),
        ...
        for next in chain {
            if !seen.contains(&next) {
                diagnostics.push(ReportedDiagnostic::coded_error(
                    source_path.clone(),
                    format!("fallback {from} → unknown LocaleId {next}"),
                    DIAG_LOCALE_FALLBACK_UNKNOWN,
                ));
                continue;
            }
