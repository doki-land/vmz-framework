...
fn bind_object_assignment_target(
    v: &mut FieldRw,
    obj: &oxc::ast::ast::ObjectAssignmentTarget<'_>,
    base: &DepKey,
) {
    use oxc::ast::ast::AssignmentTargetProperty as P;
    for prop in &obj.properties {
        match prop {
...
fn bind_array_assignment_target(
    v: &mut FieldRw,
    arr: &oxc::ast::ast::ArrayAssignmentTarget<'_>,
    base: &DepKey,
) {
    let root = base.root_field().to_string();
...
fn bind_assignment_maybe(
    v: &mut FieldRw,
    el: &oxc::ast::ast::AssignmentTargetMaybeDefault<'_>,
    base: &DepKey,
) {
    use oxc::ast::ast::AssignmentTargetMaybeDefault as M;
    match el {
...
fn path_from_chain_element(el: &oxc::ast::ast::ChainElement<'_>) -> Option<(String, Vec<PathSeg>)> {
    match el {
        oxc::ast::ast::ChainElement::StaticMemberExpression(m) => {
            let (root, mut segs) = path_from_object(&m.object)?;
            segs.push(PathSeg::Ident(m.property.name.to_string()));
