//! X2 DX: cross-SFC Symbol/Reference index, method/component/capability rename,
...
pub fn check_dx_x2(root: &Path) -> X2CheckReport {
    let index = build_symbol_index(root);
    let mut diagnostics = Vec::new();
    let code_actions = collect_safe_fixes(root, &mut diagnostics);
    X2CheckReport { schema: X2_CHECK_SCHEMA.into(), index, code_actions, diagnostics }
}
...
pub fn plan_x2_rename(root: &Path, intent: &RenameIntent, kind: &str) -> WorkspaceEditPlan {
    ...
    if plan.edits.is_empty() {
        ...
        plan.diagnostics.push(DxDiagnostic {
            path: String::new(),
            severity: "error".into(),
            message: format!("no proven X2 references for {kind} `{from}`"),
    ...
    plan.diagnostics.push(DxDiagnostic {
        ...
        severity: "info".into(),
        message: format!(
            "X2 rename ready: {kind} `{from}` -> `{to}` ({} edit(s), {refs_n} ref(s))",
            plan.edits.len()
        ),
...
"#,
        ...
        let report = check_dx_x2(&root);
        assert!(
            report.code_actions.iter().any(|a| a.kind == "safe_fix"),
            "{:?}",
            report.code_actions
        );
        assert!(
            report.diagnostics.iter().any(|d| d.code.as_deref() == Some(DIAG_CLASS_NAME_MISMATCH))
        );
        let _ = fs::remove_dir_all(&root);
    }
}
