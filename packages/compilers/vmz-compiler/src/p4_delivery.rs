//! P4: Delivery Proof algebraic check (doc 13 §4.4 / §4.12 / §4.14).
//!
...
/// Validate a DeliveryProofScenario against P4 hard contracts.
...
/// P4 check for a workspace root (optional delivery-proof-scenario.json).
pub fn check_p4_delivery_proof(root: &Path) -> DeliveryProofCheckReport {
...
mod tests {
    ...
    fn cross_delivery_proof_ready() {
        let dir = tmp("vmz-p4-");
        let report = check_p4_delivery_proof(&dir);
    ...
    fn reject_constraint_exceeded() {
        ...
        fs::write(
            dir.join("delivery-proof-scenario.json"),
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_DELIVERY_CONSTRAINT_EXCEEDED))
        );
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn reject_plan_version_mismatch() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_HOST_PLAN_VERSION_MISMATCH))
        );
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn reject_copies_semantic_ir() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_PROOF_COPIES_SEMANTIC_IR))
        );
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn reject_update_without_reproof() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_UPDATE_WITHOUT_REPROOF))
        );
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn reject_insecure_remote() {
        ...
        let mut s = DeliveryProofScenario::cross_delivery_proof_example();
        s.units[2].delivery.asset_strategy = "remote".into();
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_SECURITY_POLICY_INSECURE))
        );
        let _ = fs::remove_dir_all(dir);
    }
}
