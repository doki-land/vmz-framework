//! MP0: target-neutral Execution Plan contract (doc 24 §3 / §14).
//!
...
/// Substrings that must never appear in Execution Plan JSON (doc 24 §3 hard rule 1).
...
/// MP0 umbrella check for a workspace root (scans `*.program.json` under dist/).
pub fn check_mp0_target_contract(root: &Path) -> TargetCheckReport {
