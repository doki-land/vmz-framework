...
//!
//! Writes `pages/**/*.wxml|wxss` under `dist/_vmz/mini-deploy/wechat/`.
...
/// Relative root of the WeChat packaging tree.
pub const WECHAT_PACK_ROOT: &str = "dist/_vmz/mini-deploy/wechat";
...
pub fn lower_miniprogram_wechat_packaging(root: &Path) -> MiniWechatPackReport {
    ...
    let pack_abs = root.join("dist").join("_vmz").join("mini-deploy").join("wechat");
        ...
        for unit in &module.units {
            ...
            match emit_wechat_page(unit, &css) {
                Ok((wxml, wxss)) => {
                    ...
                    let wxml_abs = root.join(&wxml_rel);
                    let wxss_abs = root.join(&wxss_rel);
                    let json_abs = root.join(&json_rel);
                    let page_json = json!({ "usingComponents": {} });
                    let page_json_body =
                        vmz_generator::to_pretty_json(&page_json).unwrap_or_else(|_| "{}".into());
                    for (abs, rel_path, body) in [
                        (wxml_abs, wxml_rel.as_str(), wxml.as_str()),
                        (wxss_abs, wxss_rel.as_str(), wxss.as_str()),
                        (json_abs, json_rel.as_str(), page_json_body.as_str()),
                    ] {
                        if let Err(e) = write_pack_file(&abs, &format!("{body}\n")) {
                            diagnostics.push(diag(
                                rel_path,
                                Severity::Error,
                    ...
                    pages.push(WechatPackFile {
                        ...
                        stem,
                        files: vec![wxml_rel, wxss_rel, json_rel],
                    });
                }
    ...
    let app_json_abs = root.join(&app_json_rel);
    let app_json_body = vmz_generator::to_pretty_json(&app_json).unwrap_or_else(|_| "{}".into());
    if let Err(e) = write_pack_file(&app_json_abs, &format!("{app_json_body}\n")) {
        diagnostics.push(diag(
            &app_json_rel,
            Severity::Error,
