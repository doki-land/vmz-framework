...
pub fn load_document_route_plan(project_root: impl AsRef<Path>) -> DocumentRoutePlan {
    ...
    let Some(path) = found else {
        ...
        return DocumentRoutePlan {
            ...
            diagnostics: vec![ReportedDiagnostic::coded_warning(
                rel,
                "documents.config.json|json5|ts|js missing under /documents",
                DIAG_DOCUMENT_CONFIG_MISSING,
            )],
        };
    };
        ...
        Err(e) => {
            return DocumentRoutePlan {
                ...
                diagnostics: vec![ReportedDiagnostic::coded_error(
                    rel,
                    format!("read documents config failed: {e}"),
                    DIAG_DOCUMENT_CONFIG_INVALID,
                )],
            };
        }
    };
    ...
    let file: DocumentConfigFile = match serde_json::from_value(value) {
        ...
        Err(e) => {
            return DocumentRoutePlan {
                ...
                diagnostics: vec![ReportedDiagnostic::coded_error(
                    source_path,
                    format!("documents config shape invalid: {e}"),
                    DIAG_DOCUMENT_CONFIG_INVALID,
                )],
            };
        }
...
fn normalize_document_config(file: DocumentConfigFile, source_path: String) -> DocumentRoutePlan {
    ...
    if let Some(ref d) = default_locale {
        if !locale_labels.is_empty() && !locale_labels.contains_key(d) {
            diagnostics.push(ReportedDiagnostic::coded_error(
                source_path.clone(),
                format!("defaultLocale {d} missing from locales"),
    ...
    if silent_fallback_requested {
        diagnostics.push(ReportedDiagnostic::coded_error(
            source_path.clone(),
            "silent whole-page fallback is forbidden; allow only explicit nav/metadata or per-page fallback",
            DIAG_DOCUMENT_FALLBACK_SILENT,
        ));
    }
