//! NW6: Multi-platform Host Profile freeze (doc 27 §10).
//!
    ...
    for plat in REQUIRED_MULTI_PLATFORMS {
        ...
        if adapter.kind != MULTI_PLATFORM_ADAPTER_KIND {
            out.push(diag(
                ...
                "error",
                format!(
                    "NW6 adapter kind must be `{MULTI_PLATFORM_ADAPTER_KIND}` (real Xcode/Gradle later); got `{}`",
                    adapter.kind
                ),
    ...
    for adapter in &mp.adapters {
        if !REQUIRED_MULTI_PLATFORMS.contains(&adapter.platform.as_str()) {
            out.push(diag(
                ...
                "error",
                format!(
                    "unknown platform `{}` — NW6 first version is ios|android only",
                    adapter.platform
                ),
                DIAG_INVALID_PROFILE,
            ));
...
/// NW6 check for a workspace root (optional `native-multi-platform.json`).
pub fn check_nw6_multi_platform_contract(root: &Path) -> NativeMultiPlatformCheckReport {
...
mod tests {
    ...
    fn ios_android_shared_ready() {
        let dir = tmp("vmz-nw6-");
        let report = check_nw6_multi_platform_contract(&dir);
    ...
    fn rejects_platform_semantic_fork() {
        ...
        if let Some(a) = mp.adapters.iter_mut().find(|a| a.platform == "android") {
            a.bridge_schema = "com.android.private.bridge".into();
        }
        fs::write(
            dir.join("native-multi-platform.json"),
        ...
        assert!(report.diagnostics.iter().any(|d| {
            d.code.as_deref() == Some(DIAG_PLATFORM_SEMANTIC_FORK)
                || d.code.as_deref() == Some(DIAG_PLATFORM_PRIVATE_SCHEMA)
        }));
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_adapter_as_semantic_core() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_ADAPTER_IS_SEMANTIC_CORE))
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn rejects_missing_android_adapter() {
        ...
        let mp = NativeMultiPlatformManifest {
            ...
            adapters: vec![MultiPlatformAdapter::packaging_stub("ios", &shared)],
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code.as_deref() == Some(DIAG_MISSING_PLATFORM_ADAPTER))
        );
        let _ = fs::remove_dir_all(&dir);
    }
}
