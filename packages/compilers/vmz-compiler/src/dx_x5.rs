//! X5 DX: runtime trace ↔ compile-graph StableId causal replay.
//!
//! Design: `规划设计/vmz/21` §4 / §10 X5.
//! First slice: `explain write|update` chains from `*.program.json` graph edges;
//! ingest/replay StableId-tagged traces; umbrella `check_dx_x5`.
...
use vmz_protocol::{
    CAUSAL_REPLAY_SCHEMA, CausalReplayDocument, CausalReplayMatch, DxDiagnostic, EXPLAIN_SCHEMA,
    ExplainDocument, ExplainEdge, StableId, TRACE_SCHEMA, TraceDocument, TraceEvent,
    X5_CHECK_SCHEMA, X5CheckReport,
};
...
fn chain_for_field(unit: &ProgramUnitView, field: &str) -> Vec<ExplainEdge> {
...
pub fn explain_write(out_dir: &Path, spec: &str, generation: u64) -> ExplainDocument {
    ...
    ExplainDocument {
        ...
        contributions: vec![],
        chain,
        notes: Some("X5: write → effect → field → binding (Program Graph)".into()),
    }
}
...
fn explain_binding_doc(
    ...
    ExplainDocument {
        ...
        contributions: vec![],
        chain,
        notes: Some("X5: update BindingId ← field ← effect (Program Graph)".into()),
    }
}
...
/// Umbrella X5 check over current deployment artifacts.
pub fn check_dx_x5(out_dir: &Path, generation: u64) -> X5CheckReport {
    let units = load_program_units(out_dir);
    ...
    if units.is_empty() {
        ...
        return X5CheckReport {
            schema: X5_CHECK_SCHEMA.into(),
            sample_explain: None,
            sample_replay: None,
            diagnostics,
            status: "preview".into(),
    ...
    let trace = TraceDocument {
        ...
        notes: Some("synthetic trace from Program Graph for x5_check".into()),
    };
...
mod tests {
    ...
    fn ingest_rejects_bad_kind() {
        let doc =
            ingest_runtime_trace(r#"[{"kind":"write","stableId":{"kind":"bogus","id":"x"}}]"#);
    ...
    fn chain_for_field_orders_write_then_read() {
        let unit = ProgramUnitView {
            ...
            path: PathBuf::from("x"),
            edges: vec![
                GraphEdge { kind: "reads".into(), from: "binding:0".into(), to: "n".into() },
                GraphEdge {
                    kind: "writes".into(),
        ...
        assert_eq!(chain[0].from.kind, "effect");
        assert_eq!(chain[1].to.kind, "binding");
    }
}
