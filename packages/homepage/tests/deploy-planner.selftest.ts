...
assert(staticPlan.cautions.some((c) => c.includes('advice') && c.includes('--secret')), 'default ship advice');
...
assert(secretsPlan.adapters.some((a) => a.kind === 'cloudflare-pages'), 'platform kind');
assert(secretsPlan.cautions.some((c) => c.includes('advice') && c.includes('--secret')), 'ship advice always');
...
assert(ci.commandsSuggested.some((c) => c.includes('git push')), 'push after ci');
...
assert(up.commandsSuggested.some((c) => c.includes('direct-upload')), 'upload command');
