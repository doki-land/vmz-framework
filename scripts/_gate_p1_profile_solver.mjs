/**
 * P1 gate: deterministic Surface / capability / route solver
 * (doc 13 §4.6 / §4.12 / §4.14).
 *
 * Algebraic first version — fixtures supply requirements; no real VPG infer.
 *
 * Usage (repo root): pnpm gate:p1
 */
...
function fail(msg) {
    console.error(`P1 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('P1 gate: catalog includes solver documents…');
...
console.log('P1 gate: Browser Direct smoke…');
...
console.log('P1 gate: checkP1ProfileSolverJson (browser counter)…');
const report = JSON.parse(checkP1ProfileSolverJson(dir));
...
const wsReport = JSON.parse(ws.checkP1ProfileSolver());
...
console.log('P1 gate: reject SURFACE_NO_MATCH…');
...
const noFail = JSON.parse(checkP1ProfileSolverJson(dir));
...
console.log('P1 gate: reject SURFACE_AMBIGUOUS…');
...
const ambFail = JSON.parse(checkP1ProfileSolverJson(dir));
...
console.log('P1 gate: reject CAPABILITY_UNRESOLVED…');
fs.writeFileSync(path.join(dir, 'host-profile.json'), JSON.stringify(host, null, 2));
fs.writeFileSync(path.join(dir, 'delivery-profile.json'), JSON.stringify(delivery, null, 2));
...
const capFail = JSON.parse(checkP1ProfileSolverJson(dir));
...
console.log('P1 gate: reject CAPABILITY_PERMISSION_UNDECLARED…');
...
const permFail = JSON.parse(checkP1ProfileSolverJson(dir));
...
console.log('P1 gate: reject ROUTE_UNREALIZABLE…');
...
fs.writeFileSync(path.join(dir, 'solver-input.json'), JSON.stringify(input, null, 2));
const routeFail = JSON.parse(checkP1ProfileSolverJson(dir));
...
console.log('P1 GATE OK: deterministic Surface/capability/route solve + NO_MATCH/AMBIGUOUS/UNRESOLVED diagnostics');
