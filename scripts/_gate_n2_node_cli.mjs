/**
 * N2 gate: Node `vmz build` uses N-API Workspace; rebuild path never spawns cargo/vmz-tools.
 *
...
function fail(msg) {
    console.error(`N2 GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('N2 gate: vmz build via Node bin…');
...
console.log('N2 gate: runCli check…');
const checkCode = await runCli(['check', dir]);
if (checkCode !== 0) fail(`runCli check → ${checkCode}`);

console.log('N2 gate: DevSession.rebuild must not spawn cargo/vmz-tools…');
...
console.log('N2 GATE OK: Node CLI build + Workspace rebuild (no per-rebuild CLI spawn)');
