...
function fail(msg) {
    console.error(`NW4 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('NW4 gate: catalog includes fullstack documents…');
...
console.log('NW4 gate: Browser Direct smoke…');
...
console.log('NW4 gate: checkNw4NativeFullstackContractJson…');
const report = JSON.parse(checkNw4NativeFullstackContractJson(dir));
...
const bypassFail = JSON.parse(checkNw4NativeFullstackContractJson(dir));
...
console.log('NW4 gate: reject remote SSR without integrity…');
...
const remoteFail = JSON.parse(checkNw4NativeFullstackContractJson(dir));
...
console.log('NW4 gate: reject mixed SSR cookie assumptions…');
...
const mixedFail = JSON.parse(checkNw4NativeFullstackContractJson(dir));
...
console.log('NW4 GATE OK: SSR first-paint + #server transport + auth/session + network + integrity + foul rejects');
