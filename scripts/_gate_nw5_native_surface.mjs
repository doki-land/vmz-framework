...
function fail(msg) {
    console.error(`NW5 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('NW5 gate: catalog includes native_surface documents…');
...
console.log('NW5 gate: Browser Direct smoke…');
...
console.log('NW5 gate: checkNw5NativeSurfaceContractJson…');
const report = JSON.parse(checkNw5NativeSurfaceContractJson(dir));
...
const shareFail = JSON.parse(checkNw5NativeSurfaceContractJson(dir));
...
console.log('NW5 gate: reject surface confused with capability…');
...
const confFail = JSON.parse(checkNw5NativeSurfaceContractJson(dir));
...
console.log('NW5 gate: reject semantic-truth surface…');
...
const truthFail = JSON.parse(checkNw5NativeSurfaceContractJson(dir));
...
console.log('NW5 GATE OK: NativeSurfaceId + owner/lifetime + camera.preview + surface≠capability + no implicit state share');
