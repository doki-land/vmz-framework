/**
 * X0 gate: unified DX + umbrella protocol (`vmz.protocol.v0` / `vmz.dx.v0`).
 *
 * Freezes catalog + Explain/Affected; JS `@vmz/protocol` must match native.
 * Design: `规划设计/vmz/21` §10 X0.
 *
 * Usage (repo root): pnpm gate:x0
...
function fail(msg) {
    console.error(`X0 GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('X0 gate: build + protocol / DX catalog / explain / affected…');
...
console.log('X0 GATE OK: protocol.v0 + dx catalog + explain.v0 + affected.v0');
