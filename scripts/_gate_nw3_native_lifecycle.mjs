...
function fail(msg) {
    console.error(`NW3 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('NW3 gate: catalog includes lifecycle documents…');
...
console.log('NW3 gate: Browser Direct smoke…');
...
console.log('NW3 gate: checkNw3NativeLifecycleContractJson…');
const report = JSON.parse(checkNw3NativeLifecycleContractJson(dir));
...
const wsReport = JSON.parse(ws.checkNw3NativeLifecycleContract());
if (wsReport.schema !== NATIVE_HOST_LIFECYCLE_CHECK_SCHEMA || wsReport.status !== 'ready') {
    fail(`workspace check ${wsReport.status}`);
}

console.log('NW3 gate: reject backgroundEqualsDestroy…');
...
const bgFail = JSON.parse(checkNw3NativeLifecycleContractJson(dir));
...
console.log('NW3 gate: reject crashRestoreAssumesJsHeap…');
...
const crashFail = JSON.parse(checkNw3NativeLifecycleContractJson(dir));
...
console.log('NW3 gate: reject missing lifecycle event…');
...
const missFail = JSON.parse(checkNw3NativeLifecycleContractJson(dir));
...
console.log('NW3 GATE OK: lifecycle events + persistence/update/offline + background≠destroy + crash≠js-heap');
