...
function fail(msg) {
    console.error(`NW6 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('NW6 gate: catalog includes multi_platform documents…');
...
console.log('NW6 gate: Browser Direct smoke…');
...
console.log('NW6 gate: checkNw6MultiPlatformContractJson…');
const report = JSON.parse(checkNw6MultiPlatformContractJson(dir));
...
const forkFail = JSON.parse(checkNw6MultiPlatformContractJson(dir));
...
console.log('NW6 gate: reject missing android adapter…');
...
const missFail = JSON.parse(checkNw6MultiPlatformContractJson(dir));
...
console.log('NW6 gate: reject adapter as semantic core…');
...
const coreFail = JSON.parse(checkNw6MultiPlatformContractJson(dir));
...
console.log('NW6 GATE OK: ios+android shared bridge/surface/deployment/test schemas + no platform semantic fork (packaging stubs only)');
