...
function fail(msg) {
    console.error(`N4 GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('N4 gate: full build…');
const full = ws.build();
...
console.log('N4 gate: affected rebuild (reverse edge)…');
const inc = ws2.build();
...
console.log('N4 gate: bundler adapter consumes Deployment IR…');
...
console.log('N4 gate: explain provenance…');
...
console.log('N4 gate: workspace package resolution…');
const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const pkgs = resolveWorkspacePackages(repoRoot);
if (!pkgs.some((p) => p.name === 'vmz')) {
    fail(`expected vmz package in workspace, got ${pkgs.map((p) => p.name).join(',')}`);
}

fs.rmSync(dir, { recursive: true, force: true });
console.log('N4 GATE OK: reverse edges + capability/region + islandHmr + explain/edge + session graph + packages + Rolldown');
