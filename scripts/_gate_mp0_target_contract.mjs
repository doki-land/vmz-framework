...
function fail(msg) {
    console.error(`MP0 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('MP0 gate: target protocol catalog…');
...
console.log('MP0 gate: Browser Direct build + plan scan (counter slice)…');
...
console.log('MP0 gate: checkMp0TargetContractJson…');
const report = JSON.parse(checkMp0TargetContractJson(dir));
...
const wsReport = JSON.parse(ws.checkMp0TargetContract());
...
const leaked = JSON.parse(checkMp0TargetContractJson(dir));
...
console.log('MP0 GATE OK: View Ops + profiles + artifact + plan DOM-leak diagnostics + Browser Direct baseline');
