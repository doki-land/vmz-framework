/**
 * X4 gate: deployment boundary validators / leakage / capability targets / dead graph.
 *
 * Design: `规划设计/vmz/21` §10 X4.
 *
 * Usage (repo root): pnpm gate:x4
 */
...
function fail(msg) {
    console.error(`X4 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('X4 gate: DX catalog includes boundary_validator / leakage / capability_target / dead_graph / x4_check…');
...
console.log('X4 gate: build Card (used) + Orphan (unused)…');
...
for (const name of ['queryBoundaryValidators', 'queryLeakage', 'queryCapabilityTargets', 'queryDeadGraph', 'checkDxX4']) {
    if (typeof ws[name] !== 'function') fail(`${name} missing on Workspace`);
}

console.log('X4 gate: boundary validators include route pages/index…');
...
console.log('X4 gate: dead graph includes Orphan, not Card…');
...
console.log('X4 gate: clean leakage + checkDxX4 ready…');
...
const ok = JSON.parse(ws.checkDxX4());
if (ok.schema !== DX_X4_CHECK_SCHEMA) fail(`x4 schema ${ok.schema}`);
if (ok.status !== 'ready') fail(`expected ready, got ${ok.status}: ${JSON.stringify(ok).slice(0, 800)}`);

// Negative leakage (ghostRpc) is proven in Rust unit tests (`dx_x4::tests::ghost_rpc_fails_leakage`),
// not by mutating real build artifacts in the gate.

ws.dispose();
fs.rmSync(dir, { recursive: true, force: true });
console.log('X4 GATE OK: boundary validators + clean leakage + capability targets + dead graph');
