...
function fail(msg) {
    console.error(`NW2 GATE FAIL: ${msg}`);
    process.exit(1);
}

console.log('NW2 gate: catalog includes capability_call + stubs…');
...
console.log('NW2 gate: Browser Direct smoke (Web artifact still baseline)…');
...
console.log('NW2 gate: checkNw2NativeBridgeContractJson (example stubs + camera call)…');
const report = JSON.parse(checkNw2NativeBridgeContractJson(dir));
...
const nonceFail = JSON.parse(checkNw2NativeBridgeContractJson(dir));
...
console.log('NW2 gate: reject not-allowlisted capability…');
fs.writeFileSync(
    path.join(dir, 'native-bridge.calls.json'),
    JSON.stringify([
        {
...
const allowFail = JSON.parse(checkNw2NativeBridgeContractJson(dir));
...
console.log('NW2 gate: reject arbitrary bridge foul…');
...
const foul = JSON.parse(checkNw2NativeBridgeContractJson(dir));
...
console.log('NW2 GATE OK: capability_call + first-batch stubs + origin/nonce/cancel/trace + foul rejects');
