/**
 * M0 gate: Application Collection / Mount contract.
 *
 ...
 * Design: `规划设计/vmz/22` §10 M0.
 *
 * Usage (repo root): pnpm gate:m0
...
function fail(msg) {
    console.error(`M0 GATE FAIL: ${msg}`);
    process.exit(1);
}
...
console.log('M0 gate: protocol catalog…');
let catalog;
try {
...
console.log('M0 gate: happy path + catalog order…');
...
console.log('M0 gate: directory name is not ApplicationId…');
...
console.log('M0 gate: mount collision + unknown reference…');
...
console.log('M0 gate: duplicate ApplicationId…');
...
console.log('M0 gate: CLI application check…');
...
console.log('M0 GATE OK: schemas + ApplicationId resolution + mount collision + CLI');
